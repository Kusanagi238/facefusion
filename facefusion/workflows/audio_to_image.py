...
def prepare_image() -> ErrorCode:
    ...
    # Remove any existing temporary target file to avoid ffmpeg 'already exists' error
    try:
        import os
        ...
        temp_target = "{}-{}{}".format(
            base, pack_resolution(temp_image_resolution), ext
        )
        if os.path.exists(temp_target):
            os.remove(temp_target)
            logger.debug(
                "removed existing temp image: {}".format(temp_target), __name__
            )
    except Exception as e:
        logger.debug("failed to remove existing temp image: {}".format(e), __name__)
