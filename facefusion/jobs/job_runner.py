    ...
    for output_path, temp_output_paths in output_set.items():
        ...
        if are_images(temp_output_paths):
            if len(temp_output_paths) == 1:
                ...
                # Multiple image outputs for the same output_path: avoid overwriting the same filename.
                # Create unique destination filenames by inserting an index before the extension.
                base, dot, ext = output_path.rpartition(".")
                if dot == "":
                    base = output_path
                    ext = ""
                else:
                    ext = "." + ext
                for idx, temp_output_path in enumerate(temp_output_paths):
                    dest = f"{base}_{idx}{ext}"
                    if not move_file(temp_output_path, dest):
                        return False
    return True
